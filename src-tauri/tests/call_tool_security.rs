...
#[test]
fn read_file_denies_explicit_external_absolute_path() {
    // 与 python 合规基准对齐：默认 workspace_only 策略拒绝工作区外的
    // 显式绝对路径读取（old/tests/compliance/test_security.py）。
    let fx = malicious_fixture();
    let out = invoke(
        &ctx_for(&fx.root),
        "read_file",
    ...
    assert_eq!(err["error"]["code"], "FILESYSTEM_ACCESS_DENIED");
}

#[test]
fn external_read_tools_deny_directory_listing_and_search() {
    let fx = malicious_fixture();
    let ctx = ctx_for(&fx.root);
    let parent = fx.outside_secret.parent().expect("外部目录");
    let parent_text = parent.to_string_lossy().to_string();

    for tool in ["list_dir", "list_files", "search_text"] {
        let out = invoke(
            &ctx,
            tool,
            json!({"path": parent_text, "query": "TOP_SECRET", "patterns": ["**/*"]}),
        );
        let err = assert_err(&out);
        assert_eq!(
            err["error"]["code"],
            "FILESYSTEM_ACCESS_DENIED",
            "tool {tool} must deny external path"
        );
    }
}

#[test]
fn view_image_denies_explicit_external_path() {
    let fx = malicious_fixture();
    let image_path = fx
        .outside_secret
        .parent()
