...
pub fn definition(name: &str) -> Option<Value> {
    ...
    let (title, description, read_only, destructive) = match name {
        ...
        "paseo_list_pending_permissions" => ("Paseo pending permissions", "Read pending Paseo permission requests; approval and denial are intentionally unavailable.", true, false),
